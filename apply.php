...
if (isset($_GET)) {
	...
	if ($total >= $eligibility) {
		...
		header("Location: view-job-post.php?id=$_GET[id]");
		$_SESSION['status'] = "You are not eligible for this drive due to the overall percentage criteria. If you think you are eligible you can contact the placement officer.";
		$_SESSION['status_code'] = "success";
	}
} else {
	header("Location: user/index.php");
